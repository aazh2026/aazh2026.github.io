...
**反借口表格**：记录"这一步人类/AI 会找什么借口跳过，以及为什么那是错的"。这不是建议——这是把工程纪律编码成可执行步骤的核心设计。

**验证非协商**：每步都有明确的退出证据要求。"看起来对"不够；必须有证据——测试通过、构建输出、运行时数据。

**渐进披露**：SKILL.md 是入口，参考资料只在需要时加载。这保持 token 使用最小化。
