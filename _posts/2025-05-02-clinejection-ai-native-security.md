...
<img src="/assets/images/2025-05-02-security-01-rings.svg" alt="框架概述" width="100%" loading="lazy" decoding="async">

<img src="/assets/images/2025-05-02-security-02-workflow.svg" alt="框架概述" width="100%" loading="lazy" decoding="async">
...
三环模型的价值不在于每一环的独立能力，而在于**环与环之间的信号流动与反馈闭环**。以一个代码生成场景为例：用户请求"优化这个排序函数并推送到生产分支"——这句话进入第一环时，感知监测层对其做语义风险评分，同时计算上下文完整性指纹。如果风险信号超过阈值，这一信息会以结构化元数据的形式传递给第二环，告知"这是一条涉及生产环境写操作的高风险指令"。

第二环收到第一环的标记后，不是简单地放行或拦截，而是对指令进行**意图解构**：将"优化排序函数"（低风险）与"推送到生产分支"（高风险）拆分开来。对于前者，渐进式权限模型授予标准执行权限；对于后者，触发行为沙箱进行对抗性验证。第二环输出的不是一个布尔值，而是一组**带条件的风险决策**：允许优化执行，但要求沙箱验证后由人工确认推送操作。

第三环接收第二环的决策，构造沙箱执行环境并注入生产分支的镜像状态。沙箱中的执行结果——包括生成的代码 diff、权限调用记录、副作用影响范围——全部被实时监控并与第二环的预期对比。如果实际行为超出预期边界，第四环的反馈回路将触发**影响回滚**：快照机制立即将系统恢复到执行前状态，同时将这条轨迹作为新样本写回第一环的风险模型。

整个循环的响应时间控制在 100ms 以内，对用户几乎透明。
...
**工具与资源**
- [AI安全检测工具箱](https://github.com/ai-security-toolkit)
- [Prompt注入防御库](https://github.com/prompt-defense)
- [AI行为监控开源项目](https://github.com/ai-behavior-monitor)
