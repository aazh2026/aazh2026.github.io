...
>
> 本文核心观点：
> 1. **遗留系统的真正债务是认知债务** — 代码只是冰山一角，水面下是40年的隐性知识，AI无法自动提取
> 2. **三层债务模型** — 技术债务（20%，AI可解）、认知债务（60%，需人类领域知识）、契约债务（20%，需社会工程学）
> 3. **渐进式演进优于彻底重写** — 历史一再证明，绞杀者模式比重写更安全、成本更低
> 4. **AI的角色是辅助，不是替代** — AI可以加速技术债务的清理，但认知债务的解决必须依靠人与人的访谈、知识萃取和社会协作

"某企业（代表性场景，未指名）曾用数月时间和大量预算，借助 AI'重写'核心交易系统。系统上线后，开发者发现业务逻辑错误明显增加，但性能数据反而改善。问题不在于 AI，而在于他们试图用 AI 解决一个根本不是技术的问题。"
...
业界已观察到的早期案例中，某大型保险公司（代表场景，未指名）曾启动雄心勃勃的项目：用 AI 辅助重写他们的核心保单系统。这个系统有数十年历史，包含数千万行 COBOL 代码，承载着公司大部分业务。

项目团队信心满满。他们使用了业界主流的 AI 代码转换工具，可以将 COBOL 自动转换为 Java。AI 还能"理解"业务逻辑，生成相应的单元测试。

经过一年多的时间，项目"成功"交付。绝大部分 COBOL 代码被转换成 Java，测试覆盖率达到了较高水平，性能也有了明显提升。

但上线后，业务侧很快发现了若干问题：

- 某些特殊业务规则的折扣计算逻辑被错误转换，导致可量化的财务损失
- 与第三方系统的集成接口发生了微妙的变化，影响了多个下游系统
- 某些只在罕见条件下触发的边界条件被遗漏，引发批处理或结算异常

**事后调查发现，问题不在于AI的能力，而在于一个根本性的误解**：遗留系统的复杂性不在代码里，而在代码周围的"隐性知识"里。

> 💡 **Key Insight**
>
> 遗留系统的复杂性不在代码里，而在代码周围的"隐性知识"里。

## 核心观点：遗留系统的三层债务

<object data="/assets/images/2025-04-19-legacy-trap-01-three-layer-debt.svg" type="image/svg+xml" width="100%" aria-label="核心观点：遗留系统的三层债务" role="img"></object>
...
我提出一个**三层债务模型**（业界观察的示意性分解，具体比例因系统而异）：

| 债务类型 | 描述 | AI能解决？ | 示意占比 |
|---------|------|-----------|------|
| 技术债务 | 过时的技术栈、糟糕的代码结构 | ✅ 可以 | 一部分 |
| 认知债务 | 业务逻辑的历史沉淀、 tribal knowledge | ❌ 不能 | 主导部分 |
| 契约债务 | 与外部系统的隐性契约、边界条件 | ❌ 不能 | 余下部分 |
...
> 💡 **Key Insight**
>
> 每一次"遗留系统"危机，最危险的策略都是"彻底重写"。最安全的策略都是"渐进式演进"。

<object data="/assets/images/2025-04-19-legacy-trap-02-strangler-fig.svg" type="image/svg+xml" width="100%" aria-label="穿越周期：从巴别塔到丝绸之路" role="img"></object>
...
> 💡 **Key Insight**
>
> 绞杀者模式不是最"优雅"的策略，但是风险最低、成功概率最高的策略。AI在其中的最佳位置是辅助迁移，而非主导重写。
