...
<object data="/assets/images/2026-05-13-perplexity-agent-skills-guide-01-context-cost.svg" type="image/svg+xml" width="100%" aria-label="Zen of Python vs Zen of Skills（插图）" role="img"></object>
...
<object data="/assets/images/2026-05-13-perplexity-agent-skills-guide-02-skill-folder.svg" type="image/svg+xml" width="100%" aria-label="Skill 是一个目录（插图）" role="img"></object>
...
第一个陷阱是"过度文档化"：把模型已经内化的知识（git 命令行、HTTP 方法）写成 Skill，本质上是在收"Skill 是税"却没有获得任何收益——每个 session 都在付 Index 成本，却换不来更准确的 agent 路由。第二个陷阱是把高频全局知识塞进条件加载的 Skill 里，导致 context drift——模型在不同 Skill 切换时丢失统一语境，全局 context 应该承担这部分权重。第三个陷阱是维护一个快速变化的领域：当你发现 Skill 里的内容在你写完之前就已经过时，说明这不是 Skill 应该承载的东西——你需要的是动态检索或直接写入 runtime，而不是一个会漂移的静态 Snapshot。
...
> 每个 Skill 都是税。
>
> 每次你添加一个额外的 Skill，你冒着让每个其他 Skill 稍微变差的风险。
...
**本系列相关**
- [Skillify：如何让 AI Agent 不再犯同样的错误](#) (AI-Native Engineering)
- [Harness Engineering：AI Agent 时代真正的工程竞争在 scaffold 层](#) (AI-Native Engineering)
- [Claude Skills 完整指南：把一次性 prompt 变成可积累的工作流资产](#) (AI-Native Engineering)
- [执行已死，判断力永生](#) (AI-Native Engineering)
