...
<object data="/assets/images/2025-05-21-ai-assisted-efficiency-metrics-02-metrics-shift.svg" type="image/jpeg" width="100%" aria-label="新效能度量体系（插图）" role="img"></object>
...
长期价值还包括Bug风险成本的降低：代码覆盖率从35%提升到75%，按行业统计的Bug发现成本（测试阶段发现 vs 生产环境发现）约1:10的比例，每避免一次生产环境Bug平均节省约2万元召回成本/紧急修复成本。综合计算，重构投入约15人天（12行改25行的重构成本），一年内即可回本，此后每年持续产生正向ROI。
