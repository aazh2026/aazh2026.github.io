...
```mermaid
flowchart TB
    S1["1. 意图表达<br/>用精确的自然语言描述需求<br/>「实现一个函数，接收订单列表，返回按状态分组的统计」"]
    S2["2. AI 生成初稿<br/>让 AI 生成多个实现方案<br/>比较不同方案的优劣"]
    S3["3. 结构审查<br/>检查设计合理性<br/>验证边界条件处理<br/>确认错误处理策略"]
    S4["4. 美学打磨<br/>统一命名风格<br/>简化复杂表达式<br/>删除冗余代码"]
    S5["5. 验证与迭代<br/>测试用例验证正确性<br/>代码审查验证可读性<br/>记录设计决策"]
