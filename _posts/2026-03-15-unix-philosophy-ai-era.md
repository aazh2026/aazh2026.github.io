...
> 💡 **Key Insight**
>
> 透明性不仅是为了调试，更是为了**建立信任**。用户需要知道 Agent 做了什么、为什么这样做。
...
**设计原则**：每个 Agent 只负责一件事；优先使用结构化 Intent 而非私有格式；保持 Agent 无状态以支持重试和并行。

**接口设计**：Intent 接口应包含操作类型、输入数据、上下文和预期结果四个字段；版本化接口以支持渐进式升级；提供清晰的错误码和失败原因。

**测试策略**：单元测试覆盖单个 Agent 的核心逻辑；集成测试验证 Agent 之间的编排是否符合预期；端到端测试确保完整 Intent 管道的端到端行为。
