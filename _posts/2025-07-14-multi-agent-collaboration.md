...
消息格式通常包含：发送方、接收方、消息类型（请求/响应/通知）、内容负载、时间戳等字段。常见格式有 JSON 和 Protocol Buffers。

### 2. 任务分配

**策略 1：基于能力的分配**

根据每个 Agent 的能力标签（擅长领域、历史表现、当前负载）将任务分配给最合适的 Agent。适用于能力可量化、任务类型稳定的场景。

**策略 2：拍卖机制**

各 Agent 根据自身能力和负载对任务进行报价，主管 Agent 选择性价比最高的 Agent 执行。适用于任务紧急度不同、资源需要动态调配的场景。

### 3. 冲突解决

**解决机制：**

冲突解决策略包括：优先级裁决（预设规则决定谁先）、投票表决（多数意见胜出）、协商让步（各方让步达成折中）、第三方仲裁（引入中立 Agent 裁判）。
...
**陷阱 1：过度分解** — 将任务分解得过细，导致协调开销超过串行执行的成本。

**陷阱 2：通信风暴** — Agent 之间产生大量无效通信，网络拥堵导致性能下降。

**陷阱 3：级联失败** — 一个 Agent 的失败触发连锁反应，导致整个系统崩溃。

**陷阱 4：调试地狱** — 多 Agent 并发执行导致问题难以复现和定位。
