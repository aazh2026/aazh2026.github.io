...
**延迟飙升：** 长上下文的首次 token 时间（TTFT）随上下文长度近似线性放大；业界通常观察到数倍乃至一个数量级的差异，受模型实现与硬件影响
**成本爆炸：** API 费用一般与上下文长度近似成正比（部分模型按"读"与"写"分别计费）
**注意力稀释：** 关键信息淹没在海量噪声中，模型容易出现 [Lost in the Middle](https://arxiv.org/abs/2307.03172) 现象——Liu 等人对长上下文位置敏感性的实证研究常被引用作为参考
...
> 上表为工程示意区间，**准确率数字会随领域、查询分布与评测协议差异显著**。业界公开评测（如 [RAGAS 框架](https://docs.ragas.io/) 与 [BEIR benchmark](https://github.com/beir-cellar/beir)）显示，纯 RAG 与混合架构的差距取决于检索质量、文档分块策略与基线模型能力，不应作为统一基准。
