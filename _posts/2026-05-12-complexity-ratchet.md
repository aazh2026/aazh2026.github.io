...
<img src="/assets/images/2026-05-12-complexity-ratchet-01-hero.svg" alt="2026-05-12-complexity-ratchet-01-hero 图示" width="100%" loading="lazy" decoding="async">
...
> 💡 **Key Insight**
>
> 棘轮是单向的。添加的测试不会消失。它们锁定的正确行为不会回退。
...
复杂度棘轮的本质不是自动化，而是**不可逆的组织承诺**。每轮添加的测试锁定了正确行为，文档记录了决策上下文，评估结果量化了进步——这三样东西加在一起，构成一个只能向上走的系统。

AI 的贡献不是让这个循环跑得更快，而是让维持这个循环不再需要人类意志力的持续投入。当 90% 覆盖率从"需要纪律坚持的目标"变成"agent 的默认产出"，工程团队真正改变的不是速度，而是系统长期演化的方向。

代码库会腐化，因为腐化的摩擦小于维护的摩擦。棘轮翻转了这个不等式。
