...
让我们先看一组**业界常被引用的代表性数字**（具体数字请以最新公司披露为准）：

- **Google**: 代码总量与工程师规模在公开演讲（如 [Google Engineering Productivity Research](https://research.google/teams/software-engineering/) 与 [Wired 报道](https://www.wired.com/tag/google/)）中常被引用为"数十亿行代码 / 数万工程师"的量级，**"20 亿行 / 4 万工程师"** 是**业界叙事中的常见代表数字**，并非核实过的精确基线
- **Microsoft**: 公开演讲中常被引用的量级在"数十亿行 / 数万到十万工程师"区间，**具体数字请以 Microsoft Engineering 官方披露为准**
- **典型企业 SaaS / AI 创业公司**: 业界观察大致落在"数万到数百万行代码 / 数名到数十名工程师"的量级，**具体数字因公司而异**，不应作为通用基线
...
<img src="/assets/images/2025-04-08-codebase-intelligence-04-flow.svg" alt="Codebase Intelligence 数据流管道" width="100%" loading="lazy" decoding="async">
...
<img src="/assets/images/2025-04-08-codebase-intelligence-01-architecture.svg" alt="Codebase Intelligence 五层架构" width="100%" loading="lazy" decoding="async">
...
<img src="/assets/images/2025-04-08-codebase-intelligence-03-capabilities.svg" alt="Codebase Intelligence 五大核心能力总览图" width="100%" loading="lazy" decoding="async">
...
<img src="/assets/images/2025-04-08-codebase-intelligence-02-ascii-arch.svg" alt="服务依赖架构图" width="100%" loading="lazy" decoding="async">
...
<img src="/assets/images/2025-04-08-codebase-intelligence-01-impact-analysis.svg" alt="Impact Analysis 影响分析" width="100%" loading="lazy" decoding="async">
...
把"代码库作为可推理的知识系统"从概念落到工具链：

1. **15 分钟内：用一个语义搜索工具问你的代码库 3 个问题。**
   Cursor / Cody / Copilot 任选一个。问："支付失败的处理在哪里？" "谁调用了 order.cancel？" "订单状态机有哪些状态？"——对比 grep 搜索的答案，记录哪些问题 AI 答得更好。

2. **1 小时内：挑 1 个最复杂的核心模块，让 AI 生成它的架构说明。**
   把 prompt 写明："输出模块边界、对外接口、内部依赖、关键不变量。"然后人工校对一遍——这就是 Codebase Intelligence 的最小可用形态：AI 写草稿，人修一遍。

3. **本周内：给你的仓库接入一个 Tree-sitter 解析器。**
   不需要全量做知识图谱，先把 AST 跑出来——很多 IDE 插件 / LSP server 已经内置了。先把"代码可被机器理解"这一层打通，后面的语义索引和图谱才有地基。

4. **2 周内：建立"业务规则回写"机制。**
   每次从代码里发现一个隐性业务规则（折扣计算、权限判断、状态机迁移），让 AI 抽取成结构化的文档，写到 `/docs/business-rules/`。从手动 review 一两条规则开始——不要追求全自动化。

5. **1 个月内：把"新人 onboarding 第一次理解系统"的过程记录成对话。**
   让一个不熟悉项目的新人通过 AI 助手理解核心模块，记录他问了哪些问题、AI 答对了哪些、哪些答错了——这就是你的 Codebase Intelligence 系统最真实的评估数据集，比任何 benchmark 都有用。
