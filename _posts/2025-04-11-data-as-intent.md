...
*某 AI 团队（代表场景，未指名）在公开复盘中提到：他们花了相当长时间优化模型架构，准确率提升幅度有限。而另一支团队（同样代表场景，未指名），通过清理和重新标注训练数据，在数周内就获得了显著的准确率提升。不是模型不重要，而是数据更重要。这类故事在业界演讲和团队复盘中相当常见。"
...
**这不是个案。业界广泛流传的观察（多家数据科学团队调研和从业者访谈反复印证）：数据科学家大量时间花在数据准备上，用于模型开发的时间占比相对较小。**（具体百分比因团队、任务类型和项目阶段而异；可参考 [Anaconda Data Science Survey](https://www.anaconda.com/state-of-data-science-2023) 与 [CrowdFlower 数据科学行业报告](https://visit.figure-eight.com/rs/608-ZVR-991/images/Data_Science_Report_2016.pdf) 等行业调查）。
...
<object data="/assets/images/2025-04-11-data-as-intent-02-era-evolution.svg" type="image/svg+xml" width="100%" aria-label="穿越周期：从代码到模型到数据" role="img"></object>
...
<object data="/assets/images/2025-04-11-data-as-intent-01-flywheel.svg" type="image/svg+xml" width="100%" aria-label="反直觉洞察：数据-模型-应用的飞轮效应" role="img"></object>
