...
>
> 本文核心观点：
> 1. **认知天花板** — 人类评审员受制于工作记忆、注意力衰减和认知偏见，严重bug遗漏率高达57%
> 2. **历史押韵** — 质量控制每次跃迁都从"人工"走向"智能"，软件代码评审正在重演这一历程
> 3. **三种形态** — AI-Native评审演化为事前预防（Pre-commit）、事中协作（Real-time）、事后学习（Post-merge）三种形态
> 4. **人机分工** — 人类从"检查者"变为"决策者"，AI负责模式匹配，人类负责业务判断
>
> 代码评审不会消失——它会重生为更智能的形态。

*"在多家大型工程团队的内部实验中，AI 在代码评审中的表现已能与经验丰富的评审员匹敌——AI 通常能在数十秒内完成一轮审查，而人类评审员则需要数小时。在缺陷发现率上，AI 在某些场景下能匹敌甚至超过人类评审员，但具体数字因团队、代码库和评审任务类型而异。这类实验常被作为 AI 评审优势的代表案例在工程社区被反复讨论。"

---

## 那个被忽略的 Pull Request（代表性场景）

下面是一个在多家大型技术公司工程社区流传的典型场景，意在说明**单纯靠人类评审**的局限（具体当事人/公司已脱敏）：

某个看似无害的 PR：配置文件中的一个超时参数修改（例如从 30 秒改为 60 秒），代码看起来没问题，测试通过了，评审员在十几分钟内 Approve。

几天后，这个改动导致了公司历史上最大规模的故障之一。由于超时参数的不一致，微服务之间的级联调用形成了死锁，整个核心服务瘫痪数小时，损失达数百万到上千万美元。

事后调查发现，这个 PR 引入了一个微妙的分布式系统问题——一个节点等待 60 秒，另一个节点 30 秒后就会放弃，造成资源泄漏。这个问题在人类评审中被完全忽略，因为评审员只看了代码逻辑，没有考虑系统交互。

**这不是一个孤立事件。**

2023 年，GitHub 的[Octoverse 报告](https://github.blog/news-insights/octoverse/)显示，全球开发者每天创建超过 1000 万个 Pull Request。同时，业界对多个团队内部数据的研究反复显示，**相当比例（多份研究估算在 30%-50% 之间）的严重 bug 是在代码评审中被遗漏的**——这一区间的具体数字因团队规模、代码类型和评审严谨度而异，不应用单一来源直接定性。
...
| 认知限制 | 对代码评审的影响 |
|---------|-----------------|
| 工作记忆容量 | 人类同时保持 7±2 个信息单元，复杂系统交互超出认知负荷（Miller 经典认知研究） |
| 注意力衰减 | 评审时间越长注意力越分散，后段改动发现率通常显著下降（业界经验值，非统一基准） |
| 领域知识依赖 | 有效的评审需要深入的领域知识，跨团队评审效率极低 |
| 偏见与盲点 | 熟悉的作者获得更宽松的评审，熟人效应显著 |

业界观察，人类评审员在面对以下情况时几乎"失明"：
...
> 💡 **Key Insight**
>
> 人类评审员在面对以下情况时几乎"失明"：跨文件依赖、边界条件、性能隐患、安全风险。

**传统代码评审的 ROI 越来越倾向于负数**：业界观察显示，大量评审时间投入在风格讨论上，而真正严重的 bug 经常被遗漏——但遗漏率的具体数字（业界普遍报告在 30%-60% 区间）因团队、代码库和评审严谨度高度可变，不应用单一来源直接定性。
...
> 💡 **Key Insight**
>
> AI持续监控已合并的代码：识别"代码异味"的演化趋势、发现跨PR的累积风险、自动提炼可复用的知识。人类从"检查者"变成"决策者"。
...
把"代码评审的死亡"从隐喻变成你团队明天的具体动作：

1. **15 分钟内：盘点本周内被 Approve 的 5 个 PR，看看你人工评审时漏掉了什么。**
   不要假设当时评审已经够好——回看一遍有没有跨文件依赖、边界条件、性能隐患、安全风险这四类被你"失明"的问题。这是认知天花板的真实案例，不是抽象概念。

2. **1 小时内：在 PR 模板里加一行——"这个改动影响哪些模块的调用方？"**
   不需要复杂工具。一行强制性问题就能让作者主动暴露跨文件影响，AI 评审基于这个列表检索调用点比"凭印象评审"靠谱得多。

3. **本周内：让 AI 工具先跑一遍你的 PR，你再评审。**
   打开 Cursor / Copilot / Claude Code 的 code review 功能，先让 AI 列问题，再打开 PR 自己看。**不要用人类先评审、AI 补充的模式——顺序反了，AI 的价值会浪费在"挑剩下的毛病"上。**

4. **2 周内：识别 3 个适合做"Real-time AI 协作"的场景，配置 IDE 内联交互。**
   比如写 API、自动列出调用点同步更新的场景；引入新依赖、分析依赖树提示冲突的场景。把"AI 弹窗确认"模式压到最低，让 AI 真正参与结对编程。

5. **1 个月内：建立"Post-merge 知识萃取"的最小闭环。**
   在主仓库加一个 `/docs/code-smells/` 目录，每月从已合并 PR 中提炼 3-5 条"被多次犯的同款问题"作为团队 Context。让 AI 在新 PR 评审时主动检索这些历史模式——代码评审的死亡，用知识资产的诞生来补足。
