...
tags: [AI-Native软件工程, Harness Engineering, Multi-Agent, Distributed Systems, Coordination]
...
>
> 本文核心观点：
> 1. **极简协调的可行性** — 16个Claude实例并行工作，没有项目经理，没有每日站会，只有简单的git同步和文件锁，7个月后交付了一个能编译Linux内核的C编译器。
> 2. **目标替代指令** — 复杂的工程任务可以用极简的协调机制完成，不是因为Agent比人类更聪明，而是因为清晰的目标替代了复杂的指令。
> 3. **同步原语足够** — 简单的git同步和文件锁足以处理大多数协调需求，无需中央调度器。
> 4. **协调作为副作用** — 当每个Agent都遵循相同的目标和反馈机制，有效的协调可以自然涌现。
...
> 💡 **Key Insight**
>
> 复杂的工程任务，可以用极简的协调机制完成。这不是因为 Agent 比人类更聪明，而是因为清晰的目标替代了复杂的指令，简单的同步原语足以处理大多数协调需求。
...
> 💡 **Key Insight**
>
> 极简基础设施的优势不在于功能强大，而在于**可理解、可调试、可预测**。当协调机制复杂到无人能理解时，它本身就成为了风险的来源。
...
<img src="/assets/images/2026-03-15-parallel-agent-coordination-01-mechanism.svg" alt="合并冲突？让 Agent 自己解决" width="100%" loading="lazy" decoding="async">
...
> 💡 **Key Insight**
>
> 协调不一定要被"设计"出来。当每个 Agent 都遵循相同的目标和反馈机制时，有效的协调可以作为**副作用**自然涌现，而无需中央调度。
...
<img src="/assets/images/2026-03-15-parallel-agent-coordination-02-centralized-vs-decentralized.svg" alt="两种模式的适用场景" width="100%" loading="lazy" decoding="async">
...
> 💡 **Key Insight**
>
> 我们可能需要重新思考"管理"的价值。16 个 Agent 没有项目经理、没有 Scrum Master、没有每日站会，交付了一个能编译 Linux 内核的编译器；而 3 个工程师加 1 个产品经理的团队还在延期。问题可能不是 Agent 太聪明，而是人类的管理太臃肿——很多"管理活动"只是仪式感，让我们感觉在做事情，而不是真的在做事情。当然，Agent 不会写 PPT、不会在项目评审会上辩护决策、不会在绩效考核时为自己争取晋升——也许这就是为什么我们还需要人类管理者。
