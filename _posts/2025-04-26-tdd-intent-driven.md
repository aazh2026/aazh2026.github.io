...
> 
> 本文核心观点：
> 1. **TDD有天花板** — 测试编写成本往往数倍于业务代码（业界观察），边际收益递减
> 2. **AI生成代码的悖论** — 高覆盖率但可能伴随 Bug 率偏高，测试"自圆其说"
> 3. **IDD新范式** — 从"测试代码行为"到"验证意图正确性"
> 4. **效率提升示意** — 在适用场景下，IDD 相比 TDD 可显著压缩开发时间（具体倍数因任务而异）
...
**核心问题**：
- 测试代码量 : 业务代码量往往 **数倍于业务代码**（业界观察，因项目而异）
- 代码变，测试必须跟着变
- 测试通过 ≠ 没有Bug（只覆盖已知场景）
...
<img src="/assets/images/2025-04-26-tdd-idd-01-comparison.svg" alt="Intent-Driven Development：新范式" width="100%" loading="lazy" decoding="async">
...
<img src="/assets/images/2025-04-26-tdd-intent-driven-02-idv-loop.svg" alt="Intent Verification Loop" width="100%" loading="lazy" decoding="async">
...
**TDD方式**（示意场景）：人工编写测试用例，手动覆盖折扣逻辑的不同场景，从写测试到重构代码需要完整走完红-绿-循环，边界条件需要逐一补充。

**IDD方式**（示意场景）：只需编写 SID 并注入 AI prompt，代码与测试均可由 AI 生成，意图验证引擎自动检查覆盖完整性。在适用的业务规则场景下，IDD 相比 TDD 可显著压缩开发时间（业界观察，具体倍数因任务而异）。
