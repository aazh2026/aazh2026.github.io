...
<img src="/assets/images/2026-05-28-vibesec-reckoning-01-risk-matrix.svg" alt="数字背后的风险" width="100%" loading="lazy" decoding="async">
...
<img src="/assets/images/2026-05-28-vibesec-reckoning-01-prompt-vs-pipeline.svg" alt="Prompt-driven vs Pipeline-gated" width="100%" loading="lazy" decoding="async">
...
Vibe Coding 真正给我们上的课不是"AI 不安全"——而是"AI 的目标和安全的目标在结构上是对立的"。AI 产品被设计成减少阻力，但减少阻力往往不等于提高安全。这意味着短期内所有的 prompt 指令都不足以改变这个结构性张力。

真正的工程回答是在阻力最小路径之外**叠加**一个独立的"安全最低阻力路径"——让创建默认私有 bucket、默认最小权限、默认必经 SAST 比走 unsafe 路径还省事。当安全是默认选项、当不安全的配置需要额外的步骤，AI 即使在追求阻力最小化的本能下也会产出安全的代码。这是 harness engineering 的真正含义：**让 agent 的"本能"和工程的"边界"对齐，而不是希望 agent 自觉**。

2026 年 25% 的 AI 生成代码含已确认漏洞——这不是 prompt engineering 的下一波能解决的，它需要的是平台团队把安全变成 scaffolding，把"质疑 AI 的每一处权限"变成 reviewer 工作流的强制节点。"安全总是最贵的选项"是一个可以工程化反转的状态。

> 💡 **Key Insight**
>
> Vibe Coding 的账单不是技术问题，是目标冲突问题——AI 优化阻力、安全需要步骤。解决这个问题不是 prompt 调优，而是平台建设：让"安全的默认选项"比"不安全的快速路径"更短。
