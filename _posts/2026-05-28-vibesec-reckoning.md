...
在 Claude、Cursor 或 Replit 中把组织的安全指南添加为「Rules」。AI agent 用它们作为指导，使安全模式从一开始就更可能发生。它们仍然需要被确定性检查备份——在部署之前，不安全的代码、暴露的 secret、过度授权或 insecure 基础设施不能通过。

**质疑 AI 建议的每一个权限**

如果工具建议把某个东西公开或分配一个宽泛的服务账号角色，停下来问为什么。阻力最小的路径和安全路径很少是同一个。

**尝试 red team prompt**

让 AI 扮演恶意者，pen test 它刚构建的东西。这个技术一致性地暴露出前瞻性 prompt 遗漏的漏洞——特别是在权限和数据暴露方面。
...
工作流程监控团队积极使用的工具和语言，每天传递新的 CVE、平台公告和安全公告。覆盖领域最重要的是：团队编写所用的语言、部署到的云平台、AI coding 工具本身，以及 CVE 数据库。

目标是简单：在披露当天了解漏洞，而不是几周后。在 42% 的所有新企业软件现在是 AI 生成或 AI 辅助的时代，加速开发的工具也是最有可能出现在新 CVE 披露中的工具。主动监控它们是拥有安全的一部分。

---

## 长期改变：从 Prompt 到 Pipeline

**把 harness 工程集成到标准 prototype 模板中**

从概率性 prompt 转向显式反馈循环。如果计算传感器（如自动化安全扫描器）触发，agentic 循环必须结构性强制模型自我纠正，直到它通过。

**把安全规则 feed 到 application builder（Cursor、Claude 等）**

把组织的技术安全规则编译成结构化上下文 markdown 文件，加载为模型必须遵守的「Rules」。它在最便宜的点捕获最常见的失误——在任何代码被 commit 之前。

**让安全路径成为容易的路径**

给构建者一个安全默认起始点。预先配置身份验证模式、私有存储默认值、secret 处理和依赖扫描的模板，减少任何人走向不安全配置的机会。
