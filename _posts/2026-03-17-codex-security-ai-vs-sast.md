...
例如，对于用户输入 `query = "SELECT * FROM users WHERE id=" + userId`，AI 会识别 `userId` 来自 HTTP 请求参数，同时检查是否存在类型转换或 SQL 转义逻辑。如果发现输入会被 `sqlite3.escape()` 处理，则判定为**低风险**而非高危。
...
沙箱环境模拟真实数据库行为，接受构造的恶意输入并观察实际影响。验证结果不受代码注释或文档干扰，直观反映漏洞可利用性。
