...
把"自解释 API"从一个口号变成团队下周的工程动作：

1. **15 分钟内：把团队主仓库的一个核心 API 的 OpenAPI/Swagger 描述补全。**
   不需要全量，只挑最常用的 1 个端点——把 request/response schema、错误码、字段语义说明写完整。AI 能不能"读懂"你的 API，就看这一段写得够不够结构化。

2. **1 小时内：让 AI 用你的 OpenAPI 描述生成一个调用示例。**
   把生成结果和你现有文档里的示例对比——如果 AI 生成的更准确、更新，这就是"代码即文档"的第一个胜利。把它贴到 PR 描述里给团队看。

3. **本周内：识别团队最常被问的 3 个 API 使用问题。**
   这些问题大概率对应文档里没说清楚或已经过时的部分。把它们转成 OpenAPI 里的字段 description，或者在代码里加更明确的命名/类型注解——别再去维护一份 Wiki。

4. **2 周内：在 CI 流程里加入"OpenAPI 与实现代码的一致性检查"。**
   用 openapi-schema-validator 或类似工具——契约变更没改代码 / 代码变更没改契约都视为不通过。契约即代码，不是口号，是 PR 必须通过的门禁。

5. **1 个月内：跑一次"AI-first onboarding"实验。**
   让一个新成员不读任何文档，直接用 Claude/Cursor/Cody 集成你的 API，记录他成功集成的耗时 vs 传统方式（读文档 + Stack Overflow + 问同事）。这个对比就是你团队推动自解释 API 的最有力数据。
