...
<object data="/assets/images/2026-05-29-deterministic-ai-programming-01-two-paths.svg" type="image/svg+xml" width="100%" aria-label="Two Paths to Deterministic AI Programming" role="img"></object>

<object data="/assets/images/2026-05-29-deterministic-ai-programming-01-two-paths-converge.svg" type="image/svg+xml" width="100%" aria-label="两个解法，一个问题（插图）" role="img"></object>
...
> 💡 **Key Insight**
>
> 当编排规模大到需要协调几十上百个并行任务时，把决策从"实时判断"变成"静态资产"，是唯一可行的出路——两条路只是选择了不同的静态载体。
...
> 💡 **Key Insight**
>
> 把 15 种职责打包成 monolith，代价是失去对每个组件的独立替换能力——这和 1990 年代 EJB 的问题是同一脉相承的。
...
> 💡 **Key Insight**
>
> Brooks 在 1975 年写"没有银弹"，他真正警告的是"把复杂性封装进黑盒、期望它自动解决问题"的思路——这套思路在 2026 年的框架化 agent harness 里依然成立。
...
这两篇文章同天出现，表面上是工具对比，实则是同一个问题的两种解法——**都在把编排从"实时判断"变成"静态资产"**。脚本化和总线化只是不同的载体，核心逻辑是一样的：确定性换可预测性，可组合性换集成度。

这个趋势真正意味着什么？当数百个 agent 并行工作、互相 review 时，每个节点的可靠性被放大了，而 Opus 4.8 诚实性的提升是这套打法成立的前提——reviewer 真的要指出问题，而不是一味点头。强模型不是 Workflow 的可选项，是它的承重墙。

两套方案都依赖模型能力持续提升这个共同的赌注。如果 LLM 遇到不可逾越的瓶颈，大规模 agent 协作本身就失去成立的前提。但只要这个赌注持续兑现，"确定性执行"的路线就会越来越成为主流——因为模型的每一次进步，都在扩大"可以托付给确定性的工作量"的边界。