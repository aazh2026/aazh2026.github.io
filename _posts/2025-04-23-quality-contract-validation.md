...
<img src="/assets/images/2025-04-23-quality-contract-validation-04-cost-curve.svg" alt="测试滞后的代价" width="100%" loading="lazy" decoding="async">
...
<img src="/assets/images/2025-04-23-quality-contract-01-stack.svg" alt="四层契约体系" width="100%" loading="lazy" decoding="async">
...
<img src="/assets/images/2025-04-23-quality-contract-02-pipeline.svg" alt="契约即测试" width="100%" loading="lazy" decoding="async">
...
<img src="/assets/images/2025-04-23-quality-contract-validation-03-ai-qc-loop.svg" alt="AI 辅助契约生成" width="100%" loading="lazy" decoding="async">
...
把"质量契约"从一个框架落到下周的 PR 流程：

1. **15 分钟内：选一个最近的 bug，挑出对应的 User Story，看测试用例是不是覆盖了。**
   90% 的情况会发现"需求里有，测试里没有"——这就是契约缺失的证据。把它当作团队推动 Quality Contract 的第一个真实案例。

2. **1 小时内：用 AI 给一个 User Story 生成 Functional Contract YAML。**
   按本文给的 prompt 模板——输入一段需求，输出 when/then 格式的验收标准。人工校对一遍，这就是从"需求到契约"的最小闭环演示。

3. **本周内：把"性能契约"写进至少 1 个核心接口。**
   不需要 k6 全套——先定义 `response_time_p95 < Xms`、`error_rate < Y%` 这两条阈值，沉淀到接口文档里。性能契约比功能契约更容易量化，先从这切入阻力最小。

4. **2 周内：在 CI 里接入 OpenAPI/Schema 驱动的契约测试。**
   Pact / Schemathesis 任选一个——每次 PR 必须通过契约测试才能合并。先覆盖最关键的 3-5 个接口，不要追求全量。契约测试是 Quality Contract 自动化最直接的抓手。

5. **1 个月内：建立"安全契约 + 合规契约"的最小集合。**
   至少定义：SQL 注入 / XSS 防护的边界用例、GDPR 数据保留期限、PCI-DSS 卡号脱敏规则各一条。不需要全量覆盖——让团队知道"安全/合规也是有契约的"这个心智模型，比具体几条规则更重要。
